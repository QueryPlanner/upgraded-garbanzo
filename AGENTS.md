...
*   **CI/CD:** GitHub Actions runs tests and lint on PRs/pushes to `main` (no automatic deploy). Deploy manually on your host with `git pull` and `docker compose up -d --build` (see `docs/DEPLOYMENT.md`).